...
function renderFavoriteCard(fav, liveStatus = null) {
    ...
    const title = isLive ? d?.livestream_title : 'Offline';
    ...
    return `
        <div class="stream-card" data-slug="${escapeHtml(fav.slug)}" style="cursor:pointer">
            <div class="card-thumbnail">
                ${thumbSrc ? `<img src="${escapeHtml(thumbSrc)}" alt="" loading="lazy">` : '<div style="width:100%;height:100%;background:rgba(255,255,255,0.03);display:flex;align-items:center;justify-content:center;color:var(--text-secondary);font-size:13px">' + (isLive ? '' : 'Offline') + '</div>'}
...
export async function mount(params, contentEl) {
    ...
    contentEl.innerHTML = `
        <div class="section-header">
            <h1 class="section-title">Favorites ${favorites.length > 0 ? `<span class="section-count">(${favorites.length})</span>` : ''}</h1>
        </div>
        <div id="favorites-grid">
            ${favorites.length === 0
                ...
                : `<div class="stream-grid">${renderCardSkeleton(favorites.length)}</div>`}
        </div>`;

    if (favorites.length === 0) return;

    // Fetch live status for each favorite
    const grid = contentEl.querySelector('#favorites-grid');
    const results = await Promise.allSettled(
        favorites.map(fav =>
            fetchChannelData(fav.slug)
                .then(r => ({ fav, liveData: r.liveData }))
                .catch(() => ({ fav, liveData: null }))
        )
    );
